...
const validateNationalIdentityNumber = (nid, lastname, dob) => {
    ...
    const getAge = () => {
        let rawDate = nid.slice(1, 7);
        let year = rawDate.slice(4);
        let month = rawDate.slice(2, 4);
        let date = rawDate.slice(0, 2);
        let userDate = new Date(year + "-" + month + "-" + date);
        let rawAge = new Date().getTime() - userDate.getTime();
        let age = new Date(rawAge).getUTCFullYear() - 1970;
        return age;
    };
